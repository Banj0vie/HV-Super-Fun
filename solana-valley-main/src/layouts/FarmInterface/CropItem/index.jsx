...
import { FARM_POSITIONS } from "../../../constants/scene_farm";
...
const CropItem = ({
  ...
  const handleMouseEnter = e => {
    ...
    if (hoverAudio && !isDisabled) {
      ...
      hoverAudio.volume = clampVolume(volumeSetting);
    ...
    if (data.seedId) handleMouseMove(e);
  };
  ...
  return (
    <div
      ...
      style={{
        ...position,
        backgroundPositionX: data.seedId && ALL_ITEMS[data.seedId] ? 0 - frameIndex * ONE_SEED_WIDTH : 0,
        backgroundPositionY:
    ...
    >
      ...
      {data.seedId && data.seedId !== 0n && (
        <div className="growth-progress">
          <div className="growth-bar" style={{ width: `${growthProgress * 100}%` }}></div>
        </div>
      )}
      ...
      {isShowGrowthStage && data.seedId && data.seedId !== 0n && (
        <div className="growth-stage-indicator">
          {Array.from({ length: 5 }, (_, i) => (
            <div key={i} className={`growth-segment ${i <= filledSegments ? "filled" : ""}`} />
          ))}
        </div>
      )}

      {/* Tooltip rendered into portal container so it aligns with transformed parents */}
      {tooltipVisible && data.seedId && data.seedId !== 0n && !isPreview && portalContainer && !isDisabled && (
        <CropTooltip container={portalContainer} pos={tooltipPos} data={data} growthProgress={growthProgress} />
      )}

      {/* Locked plot hover tooltip */}
      {tooltipVisible && isDisabled && (
        <div className="locked-plot-tooltip">Level up your farm to unlock this plot</div>
      )}
