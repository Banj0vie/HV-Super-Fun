...
export const sendTransaction = async (txOrMethod, connection, sendTransactionFn, options = {}) => {
  const { skipPreflight = false, maxRetries = 3, onSuccess = null, onError = null } = options;
  ...
  try {
    ...
    } else if (err.message.includes("Failed to simulate") || err.message.includes("simulation failed")) {
...
export const handleSimulationError = async (tx, connection, sendTransactionFn, options = {}) => {
  const { maxRetries = 2, retryDelay = 1000, onRetry = null } = options;
  ...
  try {
    ...
    if (!signature) {
      try {
        ...
        console.warn(
          "Wallet adapter sendTransaction failed, retrying via raw sign+send with skipPreflight:",
          adapterErr?.message
        );
        if (typeof window !== "undefined" && window?.solana?.signTransaction) {
          const signed = await window.solana.signTransaction(tx);
...
/**
 * Build a VersionedTransaction from an Anchor method using the lookup table.
 * @param {Object} method - Anchor method builder (has .instruction())
 * @param {Connection} connection
 * @param {PublicKey} publicKey
 * @param {PublicKey} lookupTableAddress
 * @param {string} commitment
 * @param {TransactionInstruction[]} [computeIx] - Optional compute instructions (defaults to preIx)
 * @returns {Promise<{ transaction: VersionedTransaction, blockhash: string, lastValidBlockHeight: number }>}
 */
const buildTxWithLookupTable = async (
  method,
  connection,
  publicKey,
  lookupTableAddress,
  ...
  const instruction = await method.instruction();
  const instructions = computeIx && computeIx.length > 0 ? [...computeIx, instruction] : [...preIx, instruction];
  const { value: alt } = await connection.getAddressLookupTable(lookupTableAddress);
  if (!alt) throw new Error("Address Lookup Table not found");
...
/**
 * Simulate a signed transaction; throws if simulation fails.
 */
const simulateOrThrow = async (connection, signedTx) => {
  const sim = await connection.simulateTransaction(signedTx, {
    sigVerify: false,
    commitment: "processed",
  });
  console.log("🚀 ~ simulateOrThrow ~ sim:", sim);
  if (sim?.value?.err) {
    const simErr = sim.value.err;

    // Try to surface friendly messages for known custom errors
    try {
      let customCode = null;

      if (simErr && typeof simErr === "object" && Array.isArray(simErr.InstructionError)) {
        const [, inner] = simErr.InstructionError;
        if (inner) {
          if (typeof inner.Custom === "number") {
            customCode = inner.Custom;
          } else if (typeof inner.custom === "number") {
            // Some runtimes serialize the key as lowercase "custom"
            customCode = inner.custom;
          }
        }
      }
      ...
      if (customCode === 6026) {
        const error = new Error("Spend all game assets first.");
        error.code = 6026;
        error.simulationError = simErr;
        throw error;
      }
    } catch (e) {
      throw e;
      // Fall through to generic error below if parsing fails
    }

    const error = new Error(`Simulation failed: ${JSON.stringify(simErr)}`);
    error.simulationError = simErr;
    throw error;
  }
};
...
/**
 * Send multiple transactions with one wallet prompt (signAllTransactions).
 * Each transaction is built, then all are signed at once, then sent and confirmed sequentially.
 * @param {Object[]} methods - Array of Anchor method builders (each has .instruction())
 * @param {Connection} connection
 * @param {Function} signAllTransactionsFn - Wallet's signAllTransactions
 * @param {PublicKey} publicKey
 * @param {TransactionInstruction[]} [computeIxs] - Optional compute instructions (uses first if array, defaults to preIx)
 * @returns {Promise<string[]>} Array of transaction signatures
 */
export const sendBatchTransactionsForPhantom = async (
  methods,
  connection,
  ...
  if (!methods?.length) throw new Error("No transactions to send");

  let computeIx;
  if (computeIxs && Array.isArray(computeIxs)) {
    const filtered = computeIxs.filter(ix => ix !== undefined);
    computeIx = filtered.length > 0 ? [filtered[0]] : undefined;
  } else {
    computeIx = undefined;
  }
  ...
  const builds = await Promise.all(
    methods.map(m =>
      buildTxWithLookupTable(m, connection, publicKey, LOOKUP_TABLE_ADDRESS, "confirmed", computeIx, undefined)
    )
  );

  const signedTxs = await signAllTransactionsFn(builds.map(b => b.transaction));

  const signatures = [];
  for (let i = 0; i < signedTxs.length; i++) {
    const signed = signedTxs[i];
    const { blockhash, lastValidBlockHeight } = builds[i];
    await simulateOrThrow(connection, signed);
    ...
    if (confirmation.value?.err) {
      throw new Error(`Transaction ${i} failed: ${JSON.stringify(confirmation.value.err)}`);
    }
    signatures.push(sig);
  }
  return signatures;
