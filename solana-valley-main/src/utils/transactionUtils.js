...
export const waitForTransactionConfirmation = async (publicClient, txHash, options = {}) => {
  const { confirmations = 3, pollingInterval = 1000, timeout = 120000, onProgress = null } = options;
...
export const executeContractWrite = async (agwClient, publicClient, contractConfig, onProgress = null) => {
  ...
  try {
    ...
    const receipt = await waitForTransactionConfirmation(publicClient, txHash, {
      confirmations,
      pollingInterval,
      timeout,
      onProgress,
