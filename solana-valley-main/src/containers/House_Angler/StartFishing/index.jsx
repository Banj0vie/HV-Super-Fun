  ...
  const onConfirmBaitAmount = async amount => {
    ...
    try {
      ...
      if (result && result.txHash) {
        ...
        // Navigate to fishing page and carry the exact backend requestId
        onStart(selectedBaitId, amount, result.requestId);
      } else {
  ...
  return (
    <div className="start-fishing">
      <div className="tool-bar">
        <ItemBigView itemId={currentFishingRodId} onClick={() => {}}></ItemBigView>
        <img className="vs" src="/images/items/left-right.png" alt="vs" />
        <ItemBigView itemId={selectedBaitId} onClick={() => setIsBaitSelectDialog(true)}></ItemBigView>
      </div>
      ...
      <div className="button-wrapper">
        ...
        {selectedBaitId ? (
          ...
          <BaseButton label="Select Bait" onClick={() => setIsBaitSelectDialog(true)}></BaseButton>
        )}
      </div>
      {isBaitSelectDialog && (
        <SelectBaitDialog onClose={() => setIsBaitSelectDialog(false)} onSelect={onSelectBait}></SelectBaitDialog>
      )}
      {isConfirmDialog && (
