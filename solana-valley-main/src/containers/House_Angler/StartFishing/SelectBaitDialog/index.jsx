...
const SelectBaitDialog = ({ onClose, onSelect }) => {
  ...
  const ownedBaits = userItems.filter(item => Object.values(ID_BAIT_ITEMS).includes(item.id) && item.count > 0);
              ...
              ownedBaits.map((baitItem, index) => (
                <CardView key={index} className="p-0" secondary>
                  <div className="bait-list-item">
                    ...
                    <BaseButton className="button" label="Use" onClick={() => onSelect(baitItem.id)} small></BaseButton>
                  </div>
                </CardView>
              ))
            )}
