...
import { ID_INVENTORY_MENUS } from "../../constants/app_ids";
...
const menus = [
  ...
  { id: ID_INVENTORY_MENUS.FISHES, label: "Bait" },
...
const InventoryDialog = ({ onClose }) => {
  ...
  // Use chest opening from useChest (backend API)
  ...
  const onUseItem = async itemId => {
    ...
    try {
      ...
      if (
        Number(itemId) === ID_CHEST_ITEMS.CHEST_WOOD ||
      ...
      if (
        itemId === ID_POTION_ITEMS.POTION_GROWTH_ELIXIR ||
        ...
        if (isOnFarmPage()) {
          ...
          window.dispatchEvent(
            new CustomEvent("startPotionUsage", {
      ...
      if (
        itemId === ID_POTION_ITEMS.POTION_PESTICIDE ||
      ...
      if (
        itemId === ID_POTION_ITEMS.POTION_FERTILIZER ||
  ...
  useEffect(() => {
    switch (selectedMenu) {
      ...
      case ID_INVENTORY_MENUS.FISHES:
        const fishes = allItems.filter(
          item => item.category === "ID_ITEM_LOOT" && item.subCategory === "ID_LOOT_CATEGORY_BAIT" && item.count > 0
        );
        setList(fishes);
        break;
  ...
  return (
    <>
      <BaseDialog
        onClose={onClose}
        title="INVENTORY"
        header="/images/dialog/modal-header-inventory.png"
        headerOffset={10}
