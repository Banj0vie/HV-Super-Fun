...
const GardnerDialog = ({ onClose, label = "GARDENER", header = "/images/dialog/modal-header-gardner.png" }) => {
  const { currentLevel, maxLevel, levelUpCost, canLevelUp, levelUp, fetchGardenerData } = useGardener();
  ...
  const handleLevelUp = async () => {
    ...
    try {
      ...
      if (tx) {
        ...
        // Bypass profile cache so UI shows the new level immediately
      ...
      <div className="gardner-dialog-content">
        ...
        <CardView className="mt-1rem">
          <div className="gardner-card">
            <LabelValueBox label="Farm Plots" value={getFarmPlots(currentLevel)}></LabelValueBox>
            <LabelValueBox label="Harvest Bonus" value={`${getHarvestBonus(currentLevel)}%`}></LabelValueBox>
            <LabelValueBox label="Fishing Rod Level" value={getFishingRodLevel(currentLevel)}></LabelValueBox>
        ...
        {currentLevel < maxLevel && (
          <CardView className="">
            <div className="gardner-card">
              <LabelValueBox label="Farm Plots" value={getFarmPlots(currentLevel + 1)}>
                <span className="font-strike opacity-7">
                  {getFarmPlots(currentLevel)}
                  <span className="blue-right-triangle"></span>
                </span>
              </LabelValueBox>
              <LabelValueBox label="Harvest Bonus" value={`${getHarvestBonus(currentLevel + 1)}%`}>
                <span className="font-strike opacity-7">
                  {`${getHarvestBonus(currentLevel)}%`}
                  <span className="blue-right-triangle"></span>
                </span>
              </LabelValueBox>
              <LabelValueBox label="Fishing Rod Level" value={getFishingRodLevel(currentLevel + 1)}>
                <span className="font-strike opacity-7">
                  {getFishingRodLevel(currentLevel)}
                  <span className="blue-right-triangle"></span>
                </span>
              </LabelValueBox>
              <LabelValueBox label="Cost" value={`${levelUpCost.toFixed(2)} Honey`}></LabelValueBox>
