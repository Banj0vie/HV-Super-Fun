...
import {
  selectBankerLoading,
  selectBankerTotalGameToken,
  selectBankerTotalXGameToken,
...
const StakeHoney = ({ onBack }) => {
  ...
  const honeyBalance = parseFloat(gameToken || 0).toFixed(3);
  const xHoneyBalance = parseFloat(xTokenShare || 0).toFixed(3);
  // Est. Honey Rewards: on Stake = value of user's XHoney; on Unstake = Honey you'll receive for entered amount
  const estRewards = Number.isFinite(ratio)
    ? (isStaking ? parseFloat(xHoneyBalance) : parseFloat(amount || "0")) * ratio
    : 0;
  const estRewardsStr = Number.isFinite(estRewards) ? estRewards.toFixed(3) : "0.000";

  // Load balances and banker data on mount and when publicKey changes
  const loadData = useCallback(async () => {
    if (!publicKey) return;
    try {
      await Promise.all([getBalance(), getBankerData()]);
    } catch (err) {
  ...
  const onWithdraw = useCallback(async () => {
    ...
    try {
      const result = await unstake(parseFloat(amount));
    ...
    setData(
      isStaking
        ? [
            ...
            { label: "Est. Honey Rewards", value: `${estRewardsStr} Honey` },
          ]
        : [
            { label: "Ratio", value: `1 XHONEY - ${Number.isFinite(ratio) ? ratio.toFixed(3) : "0.000"} HONEY` },
            {
  ...
  }, [isStaking, ratio, xHoneyBalance, honeyBalance, estRewardsStr, amount]);
