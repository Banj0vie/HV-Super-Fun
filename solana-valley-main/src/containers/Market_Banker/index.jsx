...
const BankerDialog = ({ onClose, label = "VENDOR", header = "" }) => {
  const [bankerPage, setBankerPage] = useState(ID_BANKER_PAGES.STAKE_HONEY);
  return (
    <BaseDialog onClose={onClose} title={label} header={header}>
      ...
      {bankerPage === ID_BANKER_PAGES.STAKE_LP && <StakeLP onBack={() => setBankerPage(ID_BANKER_PAGES.BANKER_MENU)} />}
    </BaseDialog>
  );
};
