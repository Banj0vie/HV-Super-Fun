...
const ReferralDialog = ({ onClose, label = "REFERRAL" }) => {
  const [code, setCode] = useState("");
  const [isRegistering, setIsRegistering] = useState(false);
  const { show } = useNotification();

  // Use referral hook
  const { myReferralCode, sponsor, currentLevel, registerReferralCode, loading } = useReferral();
      ...
      <div className="referral-dialog">
        ...
        <CardView className="p-0">
          {sponsor ? (
            ...
            <div className="text-center">No sponsor</div>
          )}
        </CardView>
            ...
            {myReferralCode ? (
              <div className="text-center your-referral-code">
                <div className="share-code">
                  ...
                  <div className="highlight">{myReferralCode}</div>
                </div>
                <div className="text-1.25 margin-bottom-1rem">
                  Earn <span className="highlight">1%</span> of your referrals' spendings!
                </div>
                <BaseButton
                  className="h-4rem"
