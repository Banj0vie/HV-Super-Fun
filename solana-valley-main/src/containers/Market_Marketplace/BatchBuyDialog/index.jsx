...
const BatchBuyDialog = ({ onBack, onClose, onPurchaseSuccess, item, excludeSeller }) => {
  ...
  const findItemDataByBigIntId = bigIntIdString => {
    ...
    const matchingConstant = allItemConstants.find(constantId => constantId.toString() === bigIntIdString);
    ...
    <BaseDialog onClose={onClose} title="BATCH BUY" header="/images/dialog/modal-header-vendor.png" headerOffset={10}>
      <div className="batch-buy-dialog-content">
        <CardView className="left-panel items-list">
          <TreeInput
            onBack={onBack}
            onSelect={useCallback(checkedIds => setCheckedItemIds(checkedIds), [])}
                ...
                return (
                  <div key={item.id} className="batch-buy-item">
                    <div
                      ...
                      onClick={() => hasMultipleListings && toggleAccordion(item.id.toString())}
                    >
                      <ItemViewMarketplace item={item} onBatchBuy={handleBatchBuy} isBatchBuy />
                    </div>
                    {isExpanded && hasMultipleListings && (
                      <div className="price-details accordion-content">
                        <div className="price-summary">
                          <p>
                            <strong>Available:</strong> {item.totalAmount} items
                          </p>
                          <p>
                            <strong>Price range:</strong> {item.minPrice} - {item.maxPrice} HNY
                          </p>
                          <p>
                            <strong>Average price:</strong> {item.avgPrice} HNY
                          </p>
                          <p>
                            <strong>Listings:</strong> {item.listings.length}
                          </p>
                        </div>
                        <div className="individual-listings">
                          ...
                          {item.listings.map((listing, index) => (
                            <div key={index} className="listing-item">
                              <p>
                                Listing {index + 1}: {listing.amount} items @ {listing.pricePer} HNY each
                              </p>
                            </div>
                          ))}
                        </div>
                    ...
                    {!hasMultipleListings && (
                      <div className="price-details single-listing">
                        <p>
                          <strong>Available:</strong> {item.totalAmount} items @ {item.minPrice} HNY each
                        </p>
                      </div>
                    )}
                  </div>
