...
const WeeklyHarvest = ({ onBack }) => {
  const { sageData, fetchSageData, unlockWeeklyHarvest, getTimeUntilNextHarvestUnlock, loading, error } = useSage();
  ...
  useEffect(() => {
    if (sageData.lockedAmount > 0 && !sageData.canUnlockHarvest) {
      setRemainedTime(getTimeUntilNextHarvestUnlock());
    } else {
      setRemainedTime(0);
    }
  }, [sageData.nextHarvestUnlockTime, sageData.canUnlockHarvest, sageData.lockedAmount, getTimeUntilNextHarvestUnlock]);
