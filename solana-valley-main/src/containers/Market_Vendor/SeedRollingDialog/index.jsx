      ...
      <div className="seed-gacha-wrapper">
        <div className="seed-rolling-box-wrapper">
          {(rollingInfo.revealedSeeds || []).map((seedId, idx) => (
            <SeedRollingBox key={idx} seedPackId={seedId} delay={idx * 1000} />
          ))}
        </div>
        <div className="seed-rolling-buttons-wrapper">
          <BaseButton className="h-4rem" label="Back" onClick={onBack} small isError></BaseButton>
          <BaseButton className="h-4rem" label="Buy Again" onClick={onBuyAgain} small></BaseButton>
        </div>
      </div>
    </BaseDialog>
  );
