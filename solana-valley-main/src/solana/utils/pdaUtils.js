...
export const getRevealSeedsRemainingAccounts = (publicKey, tier) => {
...
export const getReceiverPDA = () => {
  return PublicKey.findProgramAddressSync([Buffer.from(SEEDS.RECEIVER)], SOLANA_VALLEY_PROGRAM_ID)[0];
};

export const getBankerDataPDA = () => {
  return PublicKey.findProgramAddressSync([Buffer.from(SEEDS.BANKER_DATA)], SOLANA_VALLEY_PROGRAM_ID)[0];
};
...
export const getGameRegistryPDA = () => {
  return PublicKey.findProgramAddressSync([Buffer.from(SEEDS.GAME_REGISTRY)], SOLANA_VALLEY_PROGRAM_ID)[0];
};
...
export const getRequestPDA = wallet => {
  return PublicKey.findProgramAddressSync([Buffer.from(SEEDS.REQUEST), wallet.toBuffer()], SOLANA_VALLEY_PROGRAM_ID)[0];
};

export const getFishingRequestPDA = wallet => {
  return PublicKey.findProgramAddressSync(
    [Buffer.from(SEEDS.FISHING_REQUEST), wallet.toBuffer()],
    SOLANA_VALLEY_PROGRAM_ID
  )[0];
};
...
export const getMarketDataPDA = () => {
  return PublicKey.findProgramAddressSync([Buffer.from(SEEDS.MARKET_DATA)], SOLANA_VALLEY_PROGRAM_ID)[0];
};

export const getReferralCodeOwnerPDA = code => {
  return PublicKey.findProgramAddressSync([Buffer.from(SEEDS.REF_CODE), code], SOLANA_VALLEY_PROGRAM_ID)[0];
};
...
export const getItemMintPDA = itemId => {
  ...
  return PublicKey.findProgramAddressSync(
    [Buffer.from(SEEDS.ITEM_MINT), configPublicKey.toBuffer(), new Uint8Array([category]), new Uint8Array([subtype])],
    SOLANA_VALLEY_PROGRAM_ID
  )[0];
};
...
/** Item IDs for init_user_item_ata / reclaim_user_item_ata batches. Matches contract item_ids_for_batch. */
const ITEM_IDS_ALL = (() => {
  const all = [];
  for (let sub = 1; sub <= 5; sub++) all.push((CAT_FEEBLE_SEED << 8) | sub);
  for (let sub = 1; sub <= 5; sub++) all.push((CAT_PICO_SEED << 8) | sub);
  for (let sub = 1; sub <= 12; sub++) all.push((CAT_BASIC_SEED << 8) | sub);
  for (let sub = 1; sub <= 11; sub++) all.push((CAT_PREMIUM_SEED << 8) | sub);
  for (let sub = 1; sub <= 5; sub++) all.push((CAT_PICO_PRODUCE << 8) | sub);
  for (let sub = 1; sub <= 12; sub++) all.push((CAT_BASIC_PRODUCE << 8) | sub);
  for (let sub = 1; sub <= 11; sub++) all.push((CAT_PREMIUM_PRODUCE << 8) | sub);
  all.push(ID_BAIT_ITEMS.BAIT_1, ID_BAIT_ITEMS.BAIT_2, ID_BAIT_ITEMS.BAIT_3);
  all.push(
  ...
  all.push(ID_POTION_ITEMS.POTION_GROWTH_ELIXIR, ID_POTION_ITEMS.POTION_PESTICIDE, ID_POTION_ITEMS.POTION_FERTILIZER);
  return all;
})();

const BATCH_SIZE = 8;

export const itemIdsForBatch = batch => {
  const start = batch * BATCH_SIZE;
  if (start >= ITEM_IDS_ALL.length) return [];
  const end = Math.min(start + BATCH_SIZE, ITEM_IDS_ALL.length);
  return ITEM_IDS_ALL.slice(start, end);
};

export const getInitUserItemAtaRemainingAccounts = (publicKey, batch) => {
  const itemIds = itemIdsForBatch(batch);
  const accounts = [];
  for (const itemId of itemIds) {
    const mint = getItemMintPDA(itemId);
    const ata = getAssociatedTokenAddressSync(mint, publicKey, false);
...
export const getReclaimUserItemAtaRemainingAccounts = (publicKey, batch) => {
  const itemIds = itemIdsForBatch(batch);
  const accounts = [];
  for (const itemId of itemIds) {
    const mint = getItemMintPDA(itemId);
...
export const getRequestPDAs = wallet => {
  return {
    vendorRequest: getRequestPDA(wallet),
    fishingRequest: getFishingRequestPDA(wallet),
  };
};
