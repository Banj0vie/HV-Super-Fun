...
import { fetchBalancesSuccess, updateSolBalance } from "../solana/store/slices/balanceSlice";
import { fetchUserDataSuccess, setHasProfile } from "../solana/store/slices/userSlice";
...
const serializeUserData = ud => {
  ...
  return {
    ...
    active_plot_ids: Array.isArray(ud.activePlotIds ?? ud.active_plot_ids)
      ? [...(ud.activePlotIds ?? ud.active_plot_ids)]
      : [],
...
export const useSolanaWallet = () => {
    ...
    const checkProfile = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          if (exists) {
            const serialized = serializeUserData(profile.userData || profile);
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        // User profile/userData from backend
        const walletAddress = publicKey.toString();
        const profile = await getProfile(walletAddress);
        const userData = profile ? serializeUserData(profile.userData || profile) : null;
        ...
        if (!cancelled) dispatch(updateSolBalance(solBalanceFormatted));
        ...
        try {
          ...
          gameTokenAmount = tokenAccounts.reduce((sum, acc) => {
        ...
        if (!cancelled) {
          ...
          const lockedTokensUi = (() => {
            try {
              return (parseFloat(lockedTokensStr) / TOKEN_SCALE).toString();
            } catch {
              return "0";
            }
          })();
          const xTokenShareUi = (() => {
            try {
              return (parseFloat(xTokenShareStr) / TOKEN_SCALE).toString();
            } catch {
              return "0";
            }
          })();
          dispatch(
            fetchBalancesSuccess({
              ...
              solBalance: (solBal / LAMPORTS_PER_SOL).toString(),
            })
          );
        }
      } catch (e) {
  ...
  }, [publicKey, hasProfile, connection, dispatch]);
  ...
  // Function to sign a message (used for backend auth)
  const signMessage = useCallback(
    ...
    [wallet, publicKey, signMessageAdapter]
  );
