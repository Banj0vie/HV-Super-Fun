...
const ItemCombinationBox = ({ itemId, limit = 100, limitedController = true }) => {
  ...
  const handleBaitCraft = async () => {
    ...
    if (itemId === ID_BAIT_ITEMS.BAIT_1) {
      ...
      if (itemId === ID_BAIT_ITEMS.BAIT_2) {
        result = await craftBait2(multiplier, itemIds, amounts);
      } else {
        result = await craftBait3(multiplier, itemIds, amounts);
      }
  ...
  return (
    <div className="item-combination-box">
      <ItemCombinationHeader image={itemData.image} label={itemData.label} exp={itemData.exp}></ItemCombinationHeader>
      ...
      <div className="button-wrapper">
        {!combiItems.simple && <BaseButton className="h-4rem" label="Autofill" onClick={onAutofill} small></BaseButton>}
        <BaseButton
          className="h-4rem"
