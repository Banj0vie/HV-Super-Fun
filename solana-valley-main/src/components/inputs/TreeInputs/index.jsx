...
const TreeInput = ({
      ...
      if (node.id === "ALL") {
        ...
        const getAllItemIds = () => {
          ...
          const walk = n => {
            ...
            else if (!n.children && n.id != null && n.id !== "ALL") {
              allIds.add(n.id);
            }
  ...
  const onReset = () => {
    ...
    if (itemsTree && itemsTree.length > 0) {
      ...
      const walk = node => {
        ...
        if (node.id && !node.children && !node.items && node.id !== "ALL") {
  ...
  const isNodeChecked = node => {
    if (node.id === "ALL") {
      ...
      return allItemIds.size > 0 && Array.from(allItemIds).some(id => checked.has(id));
    } else {
      ...
      return Array.from(categoryItemIds).some(id => checked.has(id));
    }
  };
    ...
    return (
      ...
      >
        {(search.length === 0 || (search.length > 0 && !hasChildren)) && (
          <div>
            <div className="tree-row" secondary={hasChildren} onClick={() => hasChildren && toggleExpand(node.id)}>
              <img src="/images/label/left-panel-normal.png" className="tree-row-bg" alt="background" />
              <div className="tree-label">{node.label}</div>
              <BaseCheckBox isChecked={isChecked} onChange={checked => toggleCheck(node, checked)} />
            </div>
          </div>
        )}
        {((search.length === 0 && hasChildren && isExpanded) || (search.length > 0 && hasChildren)) && (
          <div className="tree-children">{node.children.map(c => renderNode(c, level + 1))}</div>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="tree-input">
      <div className="tree-header">
        <BaseInput className="h-2.5rem" placeholder="Search items..." value={search} setValue={onSearch} />
        {sortable && <BaseSelect options={sortOptions} value={sortby} setValue={setSortBy} />}
      </div>
      <div className="tree-content">{filtered.map(node => renderNode(node))}</div>
